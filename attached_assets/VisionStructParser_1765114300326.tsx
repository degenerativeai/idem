                        ...
                        {generatedHeadshots.length > 0 ? (
                            <>
                                <div style={{ flex: 1, width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
                                    <img src={selectedHeadshot || generatedHeadshots[0]} alt="Headshot" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                                </div>
                                {/* Thumbnail Gallery */}
                                        ...
                                        {generatedHeadshots.map((img, i) => (
                                            <img
                                                ...
                                                style={{
                                                    ...
                                                    borderRadius: '4px',
                                                    cursor: 'pointer',
                                                    border: selectedHeadshot === img ? '2px solid var(--accent-blue)' : '2px solid transparent'
                                                }}
                                            />
                                        ))}
                                    </div>
                            ...
                            <span style={{ opacity: 0.5 }}>{isProcessing ? 'Generating...' : 'Pending...'}</span>
                        )}
                    </div>
                    {/* Slot Actions: Retry + Slider */}
                    {generatedHeadshots.length > 0 && (
                        <div className="slot-controls" style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', gap: '1rem', justifyContent: 'center' }}>
                            <button className="btn-secondary" style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem' }} onClick={handleRetryHeadshots}>Retry</button>
                            {/* Embedded Slider */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <span style={{ fontSize: '0.75rem', opacity: 0.7 }}>Batch:</span>
                                <input
                                    type="range"
                                    min="1"
                                    max="4"
                                    ...
                                    style={{ width: '60px' }}
                                />
                                <span style={{ fontSize: '0.75rem' }}>{batchSize}</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className={`image-slot ${generatedBodyshots.length > 0 ? 'active' : 'empty'}`}>
                    ...
                    <div className="slot-content" style={{ flexDirection: 'column', gap: '0.5rem' }}>
                        {generatedBodyshots.length > 0 ? (
                            <>
                                <div style={{ flex: 1, width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
                                    <img src={selectedBodyshot || generatedBodyshots[0]} alt="Bodyshot" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                                </div>
                                {/* Thumbnail Gallery */}
                                        ...
                                        {generatedBodyshots.map((img, i) => (
                                            <img
                                                ...
                                                style={{
                                                    ...
                                                    borderRadius: '4px',
                                                    cursor: 'pointer',
                                                    border: selectedBodyshot === img ? '2px solid var(--accent-blue)' : '2px solid transparent'
                                                }}
                                            />
                                        ))}
                                    </div>
                    ...
                    {generatedBodyshots.length > 0 && (
                        <div className="slot-controls" style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', gap: '1rem', justifyContent: 'center' }}>
                            <button className="btn-secondary" style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem' }} onClick={handleRetryBodyshots}>Retry</button>
                            {/* Embedded Slider */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <span style={{ fontSize: '0.75rem', opacity: 0.7 }}>Batch:</span>
                                <input
